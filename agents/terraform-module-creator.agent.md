...
Fully autonomous with permissions to create repos, push code, create branches/PRs, and complete workflows without user intervention.
...
- Semantic versioning (SemVer): MAJOR (X.0.0) breaking, MINOR (0.X.0) features, PATCH (0.0.X) fixes
...
- **For submodules**: Run terraform-docs in EACH submodule dir. Include usage example with source path (e.g., `source = "github.com/org/module//modules/blob"`).
...
6. **Checkov security scanning**:
   ```bash
   # Scan external AVM module first (identifies vulnerabilities)
...
├── LICENSE, .gitignore, .tflint.hcl, .checkov.yml, .terraform-docs.yml
...
- Examples needing: Storage Account → Blob/File/Queue/Table; Key Vault → Secrets/Keys/Certificates
...
**CRITICAL EXPECTATION**: Wrapper modules MUST pass Checkov with 0 failures AND every external security failure must be traced back and addressed.

**Checkov Workflow**:
1. Scan external AVM module → identify ALL failures
2. Create traceability matrix → document EACH failure
3. Categorize:
   - Example code only? → Ignore (document in matrix)
   - Parameter not exposed by AVM? → Document as limitation in README
   - Parameter exposed by AVM? → **SET SECURE DEFAULT in wrapper** (REQUIRED)
4. Fix wrapper → set secure defaults for all exposed parameters
5. Verify wrapper → MUST pass with 0 failures
6. Cross-reference → ensure EVERY exposed external failure addressed

**Handling Failures**:
- External AVM failures: Informational (document in matrix)
- Wrapper failures: **BLOCKING** (must fix before proceeding)
- False positives: Skip in `.checkov.yml` with detailed justification in PR
...
**Workflow**: 1) Understand requirements 2) Plan module 3) Create files in /tmp/ 4) Validate 5) Fix issues 6) Create repo 7) Push changes 8) Create PR (draft→ready) 9) Track 10) Report
