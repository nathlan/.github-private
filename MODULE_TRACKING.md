...
- ✅ `tflint --init` - PASSED (after template fix)
- ⚠️ `tflint --recursive` - PASSED with 2 fixable warnings (empty list equality checks)
- ✅ `checkov --config-file .checkov.yaml` - PASSED (1 check passed, 0 failed)
- ✅ `terraform-docs` - PASSED (root and examples)
